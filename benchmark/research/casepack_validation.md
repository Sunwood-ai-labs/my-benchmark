...
## P005 の妥当スコア
- case-pack score は `4 / 5` とする。
- 10 点表示では `8.0 / 10`。
- 満点にしない理由は、truth source と validation source の区別がまだ少し甘かったため。
- ただしローカル証拠に基づく path/version 修正、局所差分、実行 check 通過は満たしており、benchmark としては十分に discriminative に機能した。
